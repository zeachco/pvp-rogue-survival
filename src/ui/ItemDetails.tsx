/** @jsx h */
import { bucklerBlockCost, skillStatBonusDescription, weaponAttackSpeed, weaponDamage } from "../../common/combat";
import { SKILLS } from "../../common/content";
import { itemRequirementMultiplier, ITEM_PERKS, type ItemInstance, type SkillId } from "../../common/items";
...
export function itemDetails(item: ItemInstance, effectiveStats: Stats, baselineItem?: ItemInstance, baselineStats?: Stats, showRequirementPenalty = false): HTMLElement {
  ...
  return <div class="equipment-details">
    ...
    {effects.length || baselineEffects.length ? <span class="equipment-detail-wide"><small>Effects</small>{effectList(baselineEffects, effects)}</span> : null}{item.skills.length ? <span class="equipment-detail-wide"><small>Skills</small>{skillList(baselineItem?.skills ?? item.skills, item.skills)}</span> : null}
...
function skillList(current: SkillId[], projected: SkillId[]): HTMLElement {
  ...
  return <ul class="item-effect-list item-skill-list">{projected.map((skill, index) => {
    const row = itemSkillDescription(skill);
    return <li class={baseline[index] !== row.label ? "is-gain-preview" : ""} data-skill-id={skill} tabindex="0"><span>{row.label}</span><span class="tile-text-tooltip item-skill-tooltip" role="tooltip"><b>{row.label}</b><span>{row.description}</span>{row.statBonuses ? <span>{row.statBonuses}</span> : null}</span></li>;
  })}</ul> as HTMLElement;
}
export function itemSkillDescription(skill: SkillId): { label: string; description: string; statBonuses?: string } {
  return { label: SKILLS[skill].label, description: SKILLS[skill].description, statBonuses: skillStatBonusDescription(skill) || undefined };
}
function itemEffectSummary(item: ItemInstance, effectiveStats: Stats): string[] {
  ...
  for (const immunity of item.immunities ?? []) effects.push(effectiveness === 1 ? `Immune to ${capitalize(immunity)}` : `${capitalize(immunity)} immunity inactive`);
  return effects;
}

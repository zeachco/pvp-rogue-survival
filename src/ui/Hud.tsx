...
export class Hud {
	...
	private renderSpellSlot(
		...
		const button = (
			<button
				...
				type="button"
				aria-label={`${spell.label}, level ${formatPreviewValue(levelValue)}, ${spell.toggleable ? `toggleable, ${spell.active ? "enabled" : "disabled"}` : spell.passive ? "passive, always active" : spell.active ? `equipped in slot ${spell.shortcut}${spell.autoFire ? ", auto-fire enabled" : ""}` : "unequipped"}`}
		...
		button.onclick = () => {
			if (!spell.passive || spell.toggleable)
				...
				<span class="spell-tooltip-summary">
					...
					<span class="spell-tooltip-description">
						{spell.procChancesOnAttacks !== undefined
							...
							: spell.toggleable
								? `Toggleable passive — click to ${spell.active ? "disable" : "enable"}.`
								: spell.passive
	...
	private renderExtractionTooltip(
		...
		for (const { skill, currentLevel, postExtractionLevel } of levels) {
			...
			const spell = this.currentSpells.find((entry) => entry.id === skill) ?? {
				...
				toggleable: Boolean(authored.toggleable),
				autoFire: false,
	...
	private renderStaticHud(): void {
		...
		const [main, off, amulet, charm] = inspectedEquipment(build, p);
	...
	private renderPresenceSummary(): void {
		...
		this.onlineCount.append(document.createTextNode(` and you in this Realm`));
	}
...
export function inspectedEquipment(
	build: UnitBuild | undefined,
	progress: Pick<PlayerProgress, "mainHand" | "offHand" | "amulet" | "charm">,
): [
	...
	const owner = build ?? progress;
	return [owner.mainHand, owner.offHand, owner.amulet, owner.charm];
}
...
export function passiveSkillMetrics(
	...
	switch (skill) {
		case "manaShield":
			return [
				{
					label: "Damage to Mana",
					value: `${fmt(manaShieldConversionFraction(level) * 100)}%`,
				},
			];
		case "attraction":
			return [

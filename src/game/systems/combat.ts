...
function applyPhysicalAccessoryDamage(target: Unit, source: Unit | undefined, physicalDamage: number, physical: boolean, random: RandomSource): void { if (!source || !physical || physicalDamage <= 0) return; const accessory = source.offHand; const fraction = itemPhysicalBonusFraction(accessory) * (accessory ? itemRequirementMultiplier(accessory, source.stats) : 1); if (fraction > 0) target.receiveDamage(physicalDamage * fraction, random, source, false, false, { kind: "magic" }); }

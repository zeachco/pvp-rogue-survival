  ...
  for (const attack of state.attacks) {
    ...
    if (attack.owner === "hero") {
      for (const creep of state.creeps) if (creep.active && attack.contains(creep.position, creep.radius)) {
        const source = attack.source as Unit | undefined; const dealt = creep.receiveDamage(attack.damage, random, source, true, false, attack.presentation); if (dealt > 0) { applyImpactForce(creep, attack.force); if (!attack.skill) source?.restoreRage(attack.weapon?.rageCost ?? 1); } if (attack.weapon && !creep.lastHitDodged) { applyWeaponEffects(creep, attack.weapon, random, source); applyLifeSteal(source, attack.weapon, dealt); applyPhysicalAccessoryDamage(creep, source, dealt, attack.presentation.kind === "physical", random); }
        ...
        if (attack.skill === "fireBreath" && !creep.lastHitDodged) creep.addStatus({ kind: "burn", remaining: 8, damagePerSecond: 0.25 + 0.03 * ((attack.source as Unit | undefined)?.stats.spirit ?? 0), source: attack.source as Unit | undefined });
      }
    } else if (hero.active && attack.contains(hero.position, hero.radius)) {
      const dealt = hero.receiveDamage(attack.damage, random, attack.source as Unit | undefined, true, false, attack.presentation); if (dealt > 0) applyImpactForce(hero, attack.force); if (attack.weapon && !hero.lastHitDodged) { applyWeaponEffects(hero, attack.weapon, random, attack.source as Unit | undefined); applyLifeSteal(attack.source as Unit | undefined, attack.weapon, dealt); } if (attack.skill === "fireBreath" && !hero.lastHitDodged) hero.addStatus({ kind: "burn", remaining: 8, damagePerSecond: 0.25 + 0.03 * ((attack.source as Unit | undefined)?.stats.spirit ?? 0), source: attack.source as Unit | undefined });
    }
  }
  for (const projectile of state.projectiles) {
    if (!projectile.active) continue;
    if (projectile.owner === "hero") {
      const hits = state.creeps.filter((creep) => creep.active && projectile.canHit(creep.build.id) && distance(projectile.position, creep.position) <= projectile.radius + creep.radius);
      for (const hit of projectile.skill === "vampiricBoomerang" ? hits : hits.slice(0, 1)) {
        projectile.markHit(hit.build.id);
        if (projectile.skill === "frostOrb") hit.addStatus({ kind: "freeze", remaining: 4, damagePerSecond: 0, source: projectile.source });
        else { const weapon = projectile.weapon ?? equipped; const damage = projectile.skill === "vampiricBoomerang" ? projectile.damage * projectile.overlapDamageSeconds : projectile.damage; const dealt = hit.receiveDamage(damage, random, projectile.source, true, false, projectile.presentation); projectile.recordDamage(dealt); if (weapon && !hit.lastHitDodged && projectile.skill !== "vampiricBoomerang") { applyWeaponEffects(hit, weapon, random, projectile.source); applyLifeSteal(projectile.source, weapon, dealt); applyPhysicalAccessoryDamage(hit, projectile.source, dealt, projectile.presentation.kind === "physical", random); if (projectile.skill === "arcaneBolt" || projectile.skill === "frostSpike") hit.addStatus({ kind: "freeze", remaining: 4, damagePerSecond: 0, source: projectile.source }); if (projectile.skill === "rendingThrow") hit.addStatus({ kind: "bleed", remaining: 3, damagePerSecond: 0.25, source: projectile.source }); } if (dealt > 0) { applyImpactForce(hit, projectile.force); if (!projectile.skill) projectile.source?.restoreRage(weapon?.rageCost ?? 1); } if (projectile.skill !== "orbitingHammers" && projectile.skill !== "vampiricBoomerang") projectile.active = false; }
      }
      if (projectile.skill === "vampiricBoomerang") projectile.finishOverlapDamage();
    } else if (distance(projectile.position, hero.position) <= projectile.radius + hero.radius) { const weapon = projectile.weapon ?? projectile.source?.mainHand; const dealt = hero.receiveDamage(projectile.damage, random, projectile.source, true, false, projectile.presentation); if (dealt > 0) applyImpactForce(hero, projectile.force); if (weapon && !hero.lastHitDodged) { applyWeaponEffects(hero, weapon, random, projectile.source); applyLifeSteal(projectile.source, weapon, dealt); } projectile.active = false; }
...
export function applyWeaponEffects(target: Unit, item: ItemInstance, random: RandomSource, source?: Unit): void {
  ...
  if (random.next() < item.modifiers.poisonChance * effectiveness) { const voodoo = source?.isSkillOperational("voodoo") ? 1 + Math.min(1.5, source.stats.spirit * 0.03) : 1; target.addStatus({ kind: "poison", remaining: 8, damagePerSecond: (0.2 + (source?.stats.spirit ?? 0) * 0.02) * voodoo, source }); }

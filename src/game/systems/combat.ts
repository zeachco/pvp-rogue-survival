...
export function applyWeaponEffects(target: Unit, item: ItemInstance, random: RandomSource, source?: Unit): void {
  ...
  if (random.next() < item.modifiers.poisonChance * effectiveness) { const voodoo = source?.knownSkills.has("voodoo") ? 1 + Math.min(1.5, source.stats.spirit * 0.03) : 1; target.addStatus({ kind: "poison", remaining: 8, damagePerSecond: (0.2 + (source?.stats.spirit ?? 0) * 0.02) * voodoo, source }); }

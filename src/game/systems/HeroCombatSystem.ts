...
export class HeroCombatSystem {
  ...
  update(deltaSeconds: number, movementInput: Vector2, hero: Hero, state: ArenaState, progress: PlayerProgress, balance: BalanceConfig, random: RandomSource): void {
    ...
    else if (activeSkill?.id === "gravityPull") for (const creep of state.creeps) forceField(creep.position, hero.position, 180, weaponUsesProjectile(item) || item.definitionId === "staff" ? "push" : "pull");
...
export function forceField(position: Vector2, source: Vector2, distance: number, direction: "pull" | "push"): void { const dx = source.x - position.x; const dy = source.y - position.y; const length = Math.hypot(dx, dy); if (length <= 0) return; const travel = direction === "pull" ? Math.min(distance, length) : distance; const sign = direction === "pull" ? 1 : -1; position.x += dx / length * travel * sign; position.y += dy / length * travel * sign; }

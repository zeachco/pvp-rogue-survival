...
import { attackProfile, cappedSkillLevel, cooldownScale, healingCast, healingCooldown, rollAttackStrike, skillCooldown, skillDamageMultiplier, skillLabel, skillRange, spellPower, vampiricBoomerangHealingFraction, weaponUsesProjectile, whirlwindDamage, whirlwindDuration, whirlwindRadius } from "../../../common/combat";
...
export class HeroCombatSystem {
  ...
  update(deltaSeconds: number, movementInput: Vector2, hero: Hero, state: ArenaState, progress: PlayerProgress, balance: BalanceConfig, random: RandomSource): void {
    ...
    else if (activeSkill?.id === "gravityPull" && item) { const direction = weaponUsesProjectile(item) || item.definitionId === "staff" ? "push" : "pull"; for (const creep of state.creeps) forceField(creep, hero.position, 180, direction); if (direction === "push") for (const drop of state.drops) drop.applyPush(hero.position, 180); }
...
export function isSkillAvailable(progress: PlayerProgress, skill: SkillId): boolean { return progress.universalSkills.includes(skill) || Boolean(progress.mainHand?.skills.includes(skill)) || Boolean(progress.offHand?.skills.includes(skill)); }

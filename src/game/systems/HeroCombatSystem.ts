...
import { cappedSkillLevel, cooldownScale, healingCast, healingCooldown, rollWeaponStrike, skillCooldown, skillDamageMultiplier, skillLabel, skillRange, spellPower, weaponAttackSpeed, weaponRange, weaponUsesProjectile, whirlwindDamage, whirlwindDuration, whirlwindRadius } from "../../../common/combat";
...
export class HeroCombatSystem {
  ...
  update(deltaSeconds: number, movementInput: Vector2, hero: Hero, state: ArenaState, progress: PlayerProgress, balance: BalanceConfig, random: RandomSource): void {
    ...
    const manaCost = candidate ? skillManaCost(candidate.id) : 0; const staminaSkillCost = candidate?.id === "reflectiveSurge" || candidate?.id === "whirlwind" ? 3 : item.staminaCost + 0.35; const magicSkill = Boolean(candidate && SKILLS[candidate.id].resource === "mana" && hero.mana >= manaCost);
    ...
    const lifeCost = lifeSkill ? Math.min(hero.maxHp * 0.1, Math.max(0, hero.hp - 1)) : 0; hero.stamina -= staminaCost; if (magicSkill) hero.mana -= manaCost; if (lifeCost > 0) hero.takeDamage(lifeCost);
    ...
    if (activeSkill) { const duration = skillCooldown(activeSkill.id, item, effectiveStats) * cooldownScale(activeSkill.level, derived.cooldownReduction); this.skillCooldowns.set(activeSkill.id, { remaining: duration, maximum: duration }); }
  ...
  reset(): void { this.attackCooldown = 0; this.attackCooldownMax = 0; this.healingCooldown = 0; this.healingCooldownMax = 0; this.orbitCastSequence = 0; this.whirlwindRemaining = 0; this.whirlwindPulse = 0; this.skillCooldowns.clear(); }
...
function skillCostLabel(skill: SkillId, progress: PlayerProgress): string { const definition = SKILLS[skill]; if (definition.passive) return `0 ${capitalizeResource(definition.resource)}`; if (skill === "healing") return "2 Mana / HP"; if (skill === "blocking") return `${progress.offHand?.staminaCost ?? 0} Stamina`; if (definition.resource === "mana") return `${skillManaCost(skill)} Mana`; if (definition.resource === "life") return "10% Max HP"; return `${skill === "reflectiveSurge" || skill === "whirlwind" ? 3 : progress.mainHand.staminaCost + 0.35} Stamina`; }

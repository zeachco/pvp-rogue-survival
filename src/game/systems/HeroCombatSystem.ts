...
export class HeroCombatSystem {
  ...
  update(deltaSeconds: number, movementInput: Vector2, hero: Hero, state: ArenaState, progress: PlayerProgress, balance: BalanceConfig, random: RandomSource): void {
    ...
    else if (activeSkill?.id === "gravityPull") for (const creep of state.creeps) pullToward(creep.position, hero.position, 180);
...
function pullToward(position: Vector2, target: Vector2, distance: number): void { const dx = target.x - position.x; const dy = target.y - position.y; const length = Math.hypot(dx, dy); if (length <= 0) return; const travel = Math.min(distance, length); position.x += dx / length * travel; position.y += dy / length * travel; }

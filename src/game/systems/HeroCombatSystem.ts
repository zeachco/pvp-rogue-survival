...
export class HeroCombatSystem {
  ...
  update(deltaSeconds: number, movementInput: Vector2, hero: Hero, state: ArenaState, progress: PlayerProgress, balance: BalanceConfig, random: RandomSource): void {
    ...
    const item = progress.mainHand; const effectiveStats = statsWithItemBonuses(progress.stats, item, progress.offHand, progress.amulet); const derived = derivedStats(effectiveStats);
    hero.knownSkills.clear(); hero.skillLevels.clear(); for (const skill of new Set([...progress.learnedSkills, ...(item?.skills ?? []), ...(progress.offHand?.skills ?? []), ...(progress.amulet?.skills ?? [])])) if (isSkillAvailable(progress, skill) && !this.disabledSkills.has(skill)) { hero.knownSkills.add(skill); hero.skillLevels.set(skill, effectiveSkillLevel(progress, skill)); }
    const healing = healingCast(hero.hp, hero.maxHp, effectiveSkillLevel(progress, "healing")); const healingManaCost = healing.manaCost * (1 - Math.min(.9, itemResourceCostReduction(progress.offHand, "mana", effectiveStats) + itemResourceCostReduction(progress.amulet, "mana", effectiveStats)));
    ...
    const profile = attackProfile(item, effectiveStats, balance); const orderedSkills = item ? this.availableSkills(progress, item) : []; const rotatedSkills = orderedSkills.length ? [...orderedSkills.slice(this.skillPriorityCursor % orderedSkills.length), ...orderedSkills.slice(0, this.skillPriorityCursor % orderedSkills.length)] : []; const manaReduction = Math.min(.9, itemResourceCostReduction(progress.offHand, "mana", effectiveStats) + itemResourceCostReduction(progress.amulet, "mana", effectiveStats)); const lifeReduction = Math.min(.9, itemResourceCostReduction(progress.offHand, "life", effectiveStats) + itemResourceCostReduction(progress.amulet, "life", effectiveStats)); const candidate = rotatedSkills.find(({ id }) => { if ((this.skillCooldowns.get(id)?.remaining ?? 0) > 0) return false; const definition = SKILLS[id]; if (definition.resource === "mana") return hero.mana >= skillManaCost(id) * (1 - manaReduction); if (definition.resource === "life") return skillHealthRequirementMet(id, hero.hp, hero.maxHp); const cost = id === "reflectiveSurge" || id === "whirlwind" ? 3 : profile.staminaCost + .35; return hero.stamina >= cost; });
  ...
  spellSlots(progress: PlayerProgress, hero: Hero): SpellSlot[] {
    const ids = new Set<SkillId>([...progress.learnedSkills.filter((skill) => isSkillAvailable(progress, skill)), ...(progress.mainHand?.skills ?? []), ...(progress.offHand?.skills ?? []), ...(progress.amulet?.skills ?? [])]);
  ...
  private availableSkills(progress: PlayerProgress, item: ItemInstance): { id: SkillId; level: number }[] {
    ...
    for (const skill of [...item.skills, ...(progress.offHand?.skills ?? []), ...(progress.amulet?.skills ?? [])]) if (!this.disabledSkills.has(skill) && !SKILLS[skill].passive && skill !== "healing" && skill !== "blocking") skills.set(skill, effectiveSkillLevel(progress, skill));
...
export function isSkillAvailable(progress: PlayerProgress, skill: SkillId): boolean { return progress.universalSkills.includes(skill) || Boolean(progress.mainHand?.skills.includes(skill)) || Boolean(progress.offHand?.skills.includes(skill)) || Boolean(progress.amulet?.skills.includes(skill)); }
export function effectiveSkillLevel(progress: PlayerProgress, skill: SkillId): number { if (!isSkillAvailable(progress, skill)) return 0; const learned = progress.learnedSkillLevels[skill] ?? (progress.learnedSkills.includes(skill) ? 1 : 0); const equipped = progress.mainHand?.skills.includes(skill) || progress.offHand?.skills.includes(skill) || progress.amulet?.skills.includes(skill) ? 1 : 0; const stats = statsWithItemBonuses(progress.stats, progress.mainHand, progress.offHand, progress.amulet); const accessory = [progress.offHand, progress.amulet].reduce((sum, candidate) => sum + itemSkillLevelBonus(candidate, SKILLS[skill].resource) * (candidate ? itemRequirementMultiplier(candidate, stats) : 1), 0); return cappedSkillLevel(learned + equipped + Math.floor(accessory)); }

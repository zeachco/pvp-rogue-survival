...
import { cappedSkillLevel, cooldownScale, healingCast, healingCooldown, rollWeaponStrike, skillCooldown, skillDamageMultiplier, skillLabel, skillRange, spellPower, vampiricBoomerangHealingFraction, weaponAttackSpeed, weaponRange, weaponUsesProjectile, whirlwindDamage, whirlwindDuration, whirlwindRadius } from "../../../common/combat";
...
export class HeroCombatSystem {
  ...
  update(deltaSeconds: number, movementInput: Vector2, hero: Hero, state: ArenaState, progress: PlayerProgress, balance: BalanceConfig, random: RandomSource): void {
    ...
    hero.knownSkills.clear(); hero.skillLevels.clear(); for (const skill of new Set([...progress.learnedSkills, ...item.skills, ...(progress.offHand?.skills ?? [])])) if (isSkillAvailable(progress, skill) && !this.disabledSkills.has(skill)) { hero.knownSkills.add(skill); hero.skillLevels.set(skill, effectiveSkillLevel(progress, skill)); }
    const healing = healingCast(hero.hp, hero.maxHp, effectiveSkillLevel(progress, "healing")); const healingManaCost = healing.manaCost * (1 - itemResourceCostReduction(progress.offHand, "mana", effectiveStats));
    ...
    const orderedSkills = this.availableSkills(progress, item); const rotatedSkills = orderedSkills.length ? [...orderedSkills.slice(this.skillPriorityCursor % orderedSkills.length), ...orderedSkills.slice(0, this.skillPriorityCursor % orderedSkills.length)] : []; const manaReduction = itemResourceCostReduction(progress.offHand, "mana", effectiveStats); const lifeReduction = itemResourceCostReduction(progress.offHand, "life", effectiveStats); const candidate = rotatedSkills.find(({ id }) => { if ((this.skillCooldowns.get(id)?.remaining ?? 0) > 0) return false; const definition = SKILLS[id]; if (definition.resource === "mana") return hero.mana >= skillManaCost(id) * (1 - manaReduction); if (definition.resource === "life") return hero.hp > 1; const cost = id === "reflectiveSurge" || id === "whirlwind" ? 3 : item.staminaCost + .35; return hero.stamina >= cost; });
    const manaCost = candidate ? skillManaCost(candidate.id) * (1 - manaReduction) : 0; const staminaSkillCost = candidate?.id === "reflectiveSurge" || candidate?.id === "whirlwind" ? 3 : item.staminaCost + 0.35; const magicSkill = Boolean(candidate && SKILLS[candidate.id].resource === "mana" && hero.mana >= manaCost);
    ...
    const strike = rollWeaponStrike(item, effectiveStats, "hero", balance, random); const damage = (strike.damage + lifeCost) * (activeSkill ? skillDamageMultiplier(activeSkill.id) * spellPower(activeSkill.level) : 1); const presentation = { kind: activeSkill?.id === "arcaneBolt" || activeSkill?.id === "orbitingHammers" || activeSkill?.id === "frostOrb" || (!activeSkill && (item.definitionId === "staff" || item.definitionId === "scepter")) ? "magic" as const : "physical" as const, critical: strike.critical };
    ...
    else if (activeSkill?.id === "vampiricBoomerang") state.projectiles.push(Projectile.vampiricBoomerang(hero, target.position, damage, range, vampiricBoomerangHealingFraction(activeSkill.level), item));
    ...
    if (activeSkill) { const duration = skillCooldown(activeSkill.id, item, effectiveStats) * cooldownScale(activeSkill.level, Math.min(.8, derived.cooldownReduction + itemCooldownReduction(progress.offHand) * (progress.offHand ? itemRequirementMultiplier(progress.offHand, effectiveStats) : 1))); this.skillCooldowns.set(activeSkill.id, { remaining: duration, maximum: duration }); const castIndex = orderedSkills.findIndex(({ id }) => id === activeSkill.id); this.skillPriorityCursor = orderedSkills.length ? (castIndex + 1) % orderedSkills.length : 0; }
...
function skillCostLabel(skill: SkillId, progress: PlayerProgress): string { const definition = SKILLS[skill]; if (definition.passive) return `0 ${capitalizeResource(definition.resource)}`; const stats = statsWithItemBonuses(progress.stats, progress.mainHand, progress.offHand); if (skill === "healing") return `${formatCost(2 * (1 - itemResourceCostReduction(progress.offHand, "mana", stats)))} Mana / HP`; if (skill === "blocking") return `${progress.offHand?.staminaCost ?? 0} Stamina`; if (definition.resource === "mana") return `${formatCost(skillManaCost(skill) * (1 - itemResourceCostReduction(progress.offHand, "mana", stats)))} Mana`; if (definition.resource === "life") return `${formatCost((skill === "vampiricBoomerang" ? 30 : 10) * (1 - itemResourceCostReduction(progress.offHand, "life", stats)))}% Max HP`; return `${skill === "reflectiveSurge" || skill === "whirlwind" ? 3 : progress.mainHand.staminaCost + 0.35} Stamina`; }
function formatCost(value: number): string { return Number(value.toFixed(2)).toString(); }

...
function impact(
	...
	ctx.beginPath();
	ctx.arc(0, 0, 18 + radius * progress, 0, Math.PI * 2);
	ctx.stroke();
	ctx.fillStyle = color;
	for (let i = 0; i < particles; i += 1) {
		const angle = (i * Math.PI * 2) / particles;
		const distance = 20 + radius * progress * (0.55 + (i % 3) * 0.12);
		ctx.beginPath();
		ctx.arc(
...
function crescent(ctx: CanvasRenderingContext2D, progress: number): void {
	const radius = 46 + 48 * progress;
	ctx.strokeStyle = "#bafcff";
	ctx.shadowColor = "#43e6ff";
	ctx.shadowBlur = 12;
	ctx.lineWidth = 12 * (1 - progress) + 2;
	ctx.beginPath();
	ctx.arc(0, 0, radius, -1.15 + progress * 0.35, 1.15 + progress * 0.35);
	ctx.stroke();
}
function cleave(ctx: CanvasRenderingContext2D, progress: number): void {
...
function hammerCast(ctx: CanvasRenderingContext2D, progress: number): void {
...
function flurry(ctx: CanvasRenderingContext2D, progress: number): void {
...
function shockwave(ctx: CanvasRenderingContext2D, progress: number): void {
	...
	for (let ring = 0; ring < 2; ring += 1) {
		...
		ctx.beginPath();
		ctx.arc(0, 0, 20 + phase * 112, 0, Math.PI * 2);
		ctx.stroke();
	}
	ctx.globalAlpha = 1 - progress;
...
function arcane(ctx: CanvasRenderingContext2D, progress: number): void {
	ctx.fillStyle = "#8fe9ff";
	ctx.shadowColor = "#53a8ff";
	ctx.shadowBlur = 13;
	...
	ctx.lineWidth = 3 * (1 - progress) + 1;
	ctx.beginPath();
	ctx.arc(progress * 30, 0, 12 + progress * 22, 0, Math.PI * 2);
	ctx.stroke();
}
function healing(ctx: CanvasRenderingContext2D, progress: number): void {
	ctx.fillStyle = "#72f2a7";
	ctx.shadowColor = "#38d984";
	ctx.shadowBlur = 9;
	for (let i = 0; i < 9; i += 1) {
		const angle = i * 2.399;
...
function fireBreath(ctx: CanvasRenderingContext2D, progress: number): void {
	ctx.strokeStyle = "#ff6534";
	ctx.shadowColor = "#ff2f18";
	ctx.shadowBlur = 14;
	for (let index = 0; index < 5; index += 1) {
		...
		ctx.globalAlpha = (1 - phase) * (1 - index * 0.08);
		ctx.lineWidth = 7 * (1 - phase) + 2;
		ctx.beginPath();
		ctx.arc(x, 0, radius, -0.9, 0.9);
		ctx.stroke();
	}
}
function rentEdge(
	...
	ctx.shadowBlur = 8;
	const colors = ["#c91532", "#fff7ee", "#c91532"];
	for (let index = 0; index < colors.length; index += 1) {
		...
		ctx.ellipse(
			0,
			0,
			Math.max(20, range - 4 + index * 4),
			Math.max(14, range * 0.62 - 4 + index * 4),
			0,
			0,
			Math.PI * 2,
		);
...
function whirlwind(
	...
	ctx.globalAlpha = 1;
	ctx.shadowColor = "#ff765c";
	ctx.shadowBlur = 10;
	for (let edge = 0; edge < 6; edge += 1) {
		const angle = progress * Math.PI * 20 + (edge * Math.PI) / 3;
		ctx.save();
		ctx.rotate(angle);
		ctx.strokeStyle = edge % 2 ? "#fff1dc" : "#db3d2f";
		ctx.lineWidth = 3;
		ctx.beginPath();
		ctx.arc(0, 0, range - (edge % 3) * 7, -0.42, 0.42);
		ctx.stroke();
		ctx.restore();
	}
}

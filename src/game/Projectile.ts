...
export class Projectile extends GameObject {
  ...
  render(ctx: CanvasRenderingContext2D, camera: Camera): void {
    ...
    if (this.skill === "vampiricBoomerang") { ctx.rotate(Math.atan2(this.velocity.y, this.velocity.x) + this.lifetime * 10); ctx.strokeStyle = "#ff3152"; ctx.shadowColor = "#ff1838"; ctx.shadowBlur = 12; ctx.lineWidth = 6; ctx.beginPath(); ctx.arc(0, 0, 13, -.9, .9); ctx.stroke(); ctx.strokeStyle = "#850d26"; ctx.lineWidth = 2; ctx.stroke(); ctx.restore(); return; }

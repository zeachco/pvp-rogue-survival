...
export class ArenaState {
  ...
  clear(): void {
    ...
    this.pendingPickups.clear(); this.blockedPickups.clear(); this.defeatedPositions.clear(); this.waveQueue.length = 0; this.events.length = 0; this.combatTexts.length = 0; this.spellEffects.length = 0; this.swamps.length = 0;
  }
  addCombatText(text: CombatText): void { this.combatTexts.push(text); }
  updateCombatTexts(deltaSeconds: number): void { for (const text of this.combatTexts) text.age += deltaSeconds; for (let index = this.combatTexts.length - 1; index >= 0; index -= 1) if (this.combatTexts[index].age >= this.combatTexts[index].lifetime) this.combatTexts.splice(index, 1); }

...
export function emittedImpactForce(
	...
	return {
		...
		impulse: (10 + source.stats.strength * 2 + movementSpeed * 0.15) * scale,
	};
}

export function applyImpactForce(
	target: Unit,
	force: ImpactForce | undefined,
): void {
	if (!force) return;
	const direction =
		force.kind === "linear"
			? force.direction
			: {
					x: target.position.x - force.origin.x,
					y: target.position.y - force.origin.y,
				};
	if (!direction) return;

...
export class GroundSwamp extends GameObject {
	...
	render(ctx: CanvasRenderingContext2D, camera: Camera): void {
		...
		ctx.translate(x, y);
		ctx.rotate(-0.16);
		ctx.fillStyle = "rgba(21, 42, 23, .55)";
		ctx.strokeStyle = "rgba(62, 93, 50, .7)";
		ctx.lineWidth = 4;
		ctx.beginPath();
		ctx.ellipse(0, 0, this.radius, this.radius * 0.62, 0, 0, Math.PI * 2);
		ctx.fill();
		ctx.stroke();
		ctx.globalAlpha = 0.32;
		ctx.fillStyle = "#192d1b";
		for (let index = 0; index < 11; index += 1) {
			const angle = index * 2.399;
			const offset = this.radius * (0.2 + (index % 4) * 0.16);
			ctx.beginPath();
			ctx.ellipse(

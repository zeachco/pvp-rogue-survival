...
describe("server protocol integration", () => {
  test("joins, emits protocol-v19 waves, and rejects malformed commands", () => {
    ...
    expect(welcome?.config.balance.id).toBe("dev"); expect(welcome?.config.protocolVersion).toBe(19); expect(welcome?.realm.mode).toBe("training");

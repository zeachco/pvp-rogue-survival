...
describe("server protocol integration", () => {
  test("joins, emits protocol-v25 waves, and rejects malformed commands", () => {
    ...
    expect(welcome?.config.balance.id).toBe("normal"); expect(welcome?.config.protocolVersion).toBe(25); expect(welcome?.realm.mode).toBe("training");

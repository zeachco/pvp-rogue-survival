...
describe("server protocol integration", () => {
  test("joins, emits protocol-v30 waves, and rejects malformed commands", () => {
    ...
    expect(welcome?.config.balance.id).toBe("normal"); expect(welcome?.config.protocolVersion).toBe(30); expect(welcome?.realm.mode).toBe("training");

...
describe("server protocol integration", () => {
  test("joins, emits protocol-v26 waves, and rejects malformed commands", () => {
    ...
    expect(welcome?.config.balance.id).toBe("normal"); expect(welcome?.config.protocolVersion).toBe(26); expect(welcome?.realm.mode).toBe("training");

...
describe("server protocol integration", () => {
  test("joins, emits protocol-v15 waves, and rejects malformed commands", () => {
    ...
    expect(welcome?.config.balance.id).toBe("dev"); expect(welcome?.config.protocolVersion).toBe(15); expect(welcome?.realm.mode).toBe("training");

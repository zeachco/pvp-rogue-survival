...
describe("arena systems", () => {
  ...
  test("returns Vampiric Boomerang without expiring on hits and heals from recorded damage", () => { const hero = new Hero({ x: 0, y: 0 }); hero.configureStats(ZERO_STATS); hero.hp = 1; const boomerang = Projectile.vampiricBoomerang(hero, { x: 100, y: 0 }, 4, 30, .5, starterClub()); boomerang.markHit("outbound"); boomerang.recordDamage(10); expect(boomerang.active).toBeTrue(); boomerang.update(.2); boomerang.update(.2); expect(boomerang.active).toBeFalse(); expect(hero.hp).toBe(6); });

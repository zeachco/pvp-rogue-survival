...
describe("animated 3D characters", () => {
	...
	test("flashes hero and enemy presentations white for exactly one rendered frame after damage", () => {
		...
		expect(heroBody.material.color.getHex()).toBe(THEME.heroBody);
		...
		let creepBody:
			| THREE.Mesh<THREE.BufferGeometry, THREE.MeshToonMaterial>
			| undefined;
...
describe("arena systems", () => {
	...
	test("uses visible rarity colors for equipment and scrap drops", () => {
		expect(dropRarityColor("common")).toBe(RARITY_COLORS.common);
		expect(dropRarityColor("uncommon")).toBe(RARITY_COLORS.uncommon);
		expect(dropRarityColor("rare")).toBe(RARITY_COLORS.rare);
		expect(dropRarityColor("epic")).toBe(RARITY_COLORS.epic);
	});
	...
	test("generates reproducible safe columns that block units and projectiles", () => {
		...
		expect(obstacleMaterial).toBeInstanceOf(THREE.MeshToonMaterial);
		expect(obstacleMaterial.color.getHex()).toBe(THEME.pillarStone);
		const floorMaterial = arenaFloorMaterial();
		expect(floorMaterial).toBeInstanceOf(THREE.MeshToonMaterial);
		expect(floorMaterial.map).toBeInstanceOf(THREE.DataTexture);
		expect(floorMaterial.emissive.getHex()).toBe(THEME.floorEmber);
		expect(floorMaterial.emissiveIntensity).toBe(0.35);
		...
		const pillarMeshes = map.mesh.children.filter(
			(child) =>
				child instanceof THREE.Mesh && child.userData.obstacle === true,
		);
		expect(pillarMeshes).toHaveLength(15);
		expect(
			pillarMeshes.every(
				(pillar) =>
					(pillar as THREE.Mesh).material instanceof THREE.MeshToonMaterial,
			),
		).toBeTrue();
		...
		expect(map.mesh.getObjectsByProperty("type", "PointLight")).toHaveLength(6);
		expect(
			map.mesh.children.filter((child) => child instanceof THREE.InstancedMesh),
		).toHaveLength(1);
		expect(
			map.mesh.children.some(
				(child) =>
					child instanceof THREE.Mesh && child.userData.decorative === true,
			),
		).toBeTrue();
		expect(
			map.mesh.children.some(

...
describe("Bun SQL player persistence", () => {
  test("round-trips the indexed hero fields and serialized progression through SQLite", async () => {
    ...
    try {
      ...
      expect(restored?.name).toBe("Persistent"); expect(restored?.progress.level).toBe(2); expect(restored?.score).toBe(17); expect(restored?.waveNumber).toBe(2); expect(restored?.maxWaveReached).toBe(9); expect(restored?.progress.gold).toBe(88); expect(restored?.panelTriggers).toEqual({ character: false, inventory: true, multiplayer: true });

...
export const CHANGELOG_MAX_ATTEMPTS = 2;
...
type RunModel = (model: string, messages: ChatMessage[]) => Promise<string>;
...
export async function runLLM(
  ...
  timeoutMs: number = CHANGELOG_REQUEST_TIMEOUT_MS,
): Promise<string> {
  ...
  debug(
    `request ${endpoint} model=${model} messages=${messages.length} promptChars=${promptChars}`,
  );
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          `llama.cpp request timed out after ${Math.round(timeoutMs / 1000)}s.`,
          { cause: error },
        );
      throw error;
    }
    ...
    if (!response.body) throw new Error("llama.cpp returned no response body.");
    ...
    for (;;) {
      ...
      while ((newline = buffer.indexOf("\n")) !== -1) {
        ...
        try {
          ...
          // Ignore keepalive lines that are not JSON chunks.
        }
      }
    }
    if (content === "") {
      // The server may answer with a single JSON payload instead of SSE.
      try {
        const payload = JSON.parse(raw) as CompletionPayload;
        const fallback = payload.choices?.[0]?.message?.content;
        if (typeof fallback === "string") content = fallback;
      } catch {
        // Keep content empty so the explicit empty-content error below fires.
      }
    }
    if (content.trim() === "") {
      debug(
        `no completion content after ${elapsedSince(startedAt)}: raw=${raw.slice(0, 200)}`,
      );
      throw new Error("llama.cpp returned empty completion content.");
    }
    debug(
...
async function generateWeek(
  ...
  const messages: ChatMessage[] = [
    { role: "user", content: promptFor([week]) },
  ];
  for (let attempt = 1; attempt <= CHANGELOG_MAX_ATTEMPTS; attempt += 1) {
    let response: string;
    try {
      response = await runModel(CHANGELOG_MODEL, messages);
    } catch (error) {
    ...
    try {
      const periods = extractPeriods(response);
      ...
      if (attempt >= CHANGELOG_MAX_ATTEMPTS)
        throw new Error(
          `Changelog generation failed for ${week.key} with ${CHANGELOG_MODEL} after ${CHANGELOG_MAX_ATTEMPTS} attempts: ${reason}`,
        );
      messages.push(
        ...
        {
          role: "user",
          content: `Your previous response failed validation for this reason:\n${reason}\nReturn a corrected strict JSON object only.`,
        },
      );
    }
  }
...
async function runWeeks(
  weeks: WeekSource[],
  generate: (week: WeekSource) => Promise<void>,
): Promise<void> {
  const queue = [...weeks];
  await Promise.all(
    Array.from(
      { length: Math.min(CHANGELOG_CONCURRENCY, queue.length) },
      async () => {
        for (;;) {
...
export async function generatePeriods(
  ...
  for (const week of weeks) {
    ...
    toGenerate.push(week);
  }
  await runWeeks(toGenerate, (week) =>
    generateWeek(week, generated, models, runModel),
  );

...
export class FilePlayerRepository implements PlayerRepository {
  ...
  private load(): void {
    ...
    for (const saved of value.players) {
      normalizeWeapons(saved.progress); saved.progress.allocation = integerAllocation(saved.progress.allocation); saved.progress.inventoryTiles = saved.progress.inventoryTiles.filter((tile) => tile.quantity > 0); if (!saved.progress.learnedSkills.includes("rent")) saved.progress.learnedSkills.push("rent"); saved.progress.learnedSkillLevels.rent ??= 1;

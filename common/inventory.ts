...
export function collectIntoInventory(progress: PlayerProgress, item: ItemInstance, nextId: () => string, nextSeed: () => number): InventoryResult {
  ...
  if (!tile) {
    ...
    tile = { id: nextId(), key: itemStackKey(item), item, quantity: 0, automation: "upgrade" };
...
export function setAutomation(progress: PlayerProgress, tileId: string, automation: InventoryAutomation, nextId: () => string, nextSeed: () => number): InventoryResult {
  const tile = findTile(progress, tileId); if (!tile) return missing(); tile.automation = automation; runAutoUpgrades(progress, nextId, nextSeed);
  return { changed: true, reason: `${tile.item.name} set to ${automation === "upgrade" ? "Auto Upgrade" : automation}.` };
}
...
export function upgradeFromInventory(progress: PlayerProgress, tileId: string, nextId: () => string, nextSeed: () => number): InventoryResult {
  ...
  if (!existing && progress.inventoryTiles.length >= inventoryCapacity(progress.level)) return { changed: false, reason: "No permanent tile is available for the upgraded item." };
  tile.quantity -= 1; progress.gold -= gold; progress.scraps[tile.item.rarity] -= scraps;
  if (existing) existing.quantity += 1; else progress.inventoryTiles.push({ id: nextId(), key: itemStackKey(created), item: created, quantity: 1, automation: "upgrade" });
...
function storeExisting(progress: PlayerProgress, item: ItemInstance): boolean { let tile = progress.inventoryTiles.find((candidate) => candidate.key === itemStackKey(item)); if (!tile) { if (progress.inventoryTiles.length >= inventoryCapacity(progress.level)) return false; tile = { id: `tile-${item.id}`, key: itemStackKey(item), item, quantity: 0, automation: "upgrade" }; progress.inventoryTiles.push(tile); } tile.quantity += 1; return true; }

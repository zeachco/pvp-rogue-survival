...
export function equipFromInventory(progress: PlayerProgress, tileId: string): InventoryResult {
  ...
  if (progress.charm && itemStackKey(progress.charm) === tile.key) { progress.charm = undefined; return { changed: true, reason: `Unequipped ${item.name}.` }; }
  if (progress.amulet && itemStackKey(progress.amulet) === tile.key) { progress.amulet = undefined; return { changed: true, reason: `Unequipped ${item.name}.` }; }
  ...
  if (item.itemKind !== "weapon" && item.itemKind !== "amulet" && item.itemKind !== "charm" && progress.mainHand?.hands === 2) return { changed: false, reason: "A two-handed weapon cannot use an offhand item." };
  const displaced: ItemInstance[] = [];
  if (item.itemKind === "charm") { if (progress.charm) displaced.push(progress.charm); }
  else if (item.itemKind === "amulet") { if (progress.amulet) displaced.push(progress.amulet); }
  else if (item.itemKind !== "weapon") { if (progress.offHand) displaced.push(progress.offHand); }
  else {
  ...
  if (item.itemKind === "charm") progress.charm = { ...item, id: `${item.id}-equipped` };
  else if (item.itemKind === "amulet") progress.amulet = { ...item, id: `${item.id}-equipped` };
  else if (item.itemKind !== "weapon") progress.offHand = { ...item, id: `${item.id}-equipped` };
...
export function upgradeFromInventory(progress: PlayerProgress, tileId: string, nextId: () => string, nextSeed: () => number): InventoryResult {
  ...
  const upgradesEquippedCopy = tile.quantity <= equippedCopies(progress, tile); const upgradesMainHand = upgradesEquippedCopy && Boolean(progress.mainHand && itemStackKey(progress.mainHand) === tile.key); const upgradesOffHand = upgradesEquippedCopy && Boolean(progress.offHand && itemStackKey(progress.offHand) === tile.key); const upgradesAmulet = upgradesEquippedCopy && Boolean(progress.amulet && itemStackKey(progress.amulet) === tile.key); const upgradesCharm = upgradesEquippedCopy && Boolean(progress.charm && itemStackKey(progress.charm) === tile.key);
  ...
  if (upgradesAmulet) progress.amulet = { ...created, id: `${created.id}-equipped` };
  if (upgradesCharm) progress.charm = { ...created, id: `${created.id}-equipped` };
...
export function extractFromInventory(progress: PlayerProgress, tileId: string): InventoryResult {
  ...
  if (!carriedSkills.length) return { changed: false, reason: "That item has no extractable skill." };
  const universal = tile.item.rarity === "epic";
  const skills = universal ? carriedSkills : carriedSkills.filter((skill) => progress.learnedSkills.includes(skill));
  if (!universal && skills.length !== carriedSkills.length) return { changed: false, reason: `${carriedSkills.filter((skill) => !progress.learnedSkills.includes(skill)).join(", ")} must first be learned.` };
  ...
  for (const skill of skills) learnSkill(progress, skill, universal); removeEmptyInventoryTiles(progress); return { changed: true, reason: `Extracted ${skills.join(", ")} for ${cost} gold${universal ? "; available with any weapon" : ""}.` };
}
...
function equippedCopies(progress: PlayerProgress, tile: InventoryTile): number { return Number(Boolean(progress.mainHand && itemStackKey(progress.mainHand) === tile.key)) + Number(Boolean(progress.offHand && itemStackKey(progress.offHand) === tile.key)) + Number(Boolean(progress.amulet && itemStackKey(progress.amulet) === tile.key)) + Number(Boolean(progress.charm && itemStackKey(progress.charm) === tile.key)); }

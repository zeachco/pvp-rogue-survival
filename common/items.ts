...
export type ItemImmunity = "physical" | "magic" | "fire" | "frost" | "poison" | "bleed";
...
export function auraSkillForSeed(seed: number, divisor = 1): SkillId { return AURA_SKILLS[Math.abs(Math.floor(seed / divisor)) % AURA_SKILLS.length]; }
...
export const weaponLevelScale = (level: number): number => 1 + Math.max(0, level) * 0.025;
export const weaponSkillLevelScale = (level: number): number => 1 + Math.max(0, level) * 0.005;
...
export interface ItemModifiers {
  ...
  poisonChance: number; stunChance: number; lifeStealBase: number; strengthRegenMultiplier: number; goldGain: number; rarityBoost: number;
}
...
export interface ItemInstance {
  id: string; itemKind: "weapon" | "buckler" | "relic" | "amulet" | "charm"; definitionId: EquipmentDefinitionId; name: string; level: number; rarity: Rarity;
...
export function generateItem(level: number, rarity: Rarity, seed: number, filters: ItemGenerationFilters = {}): ItemInstance {
  ...
  return rollItemPerks(buildWeapon(weaponClass, level, rarity, seed, affixes, Math.floor(random() * 1e8)), seed);
}
...
export function generateAccessory(level: number, rarity: Rarity, seed: number, kind?: "amulet" | "charm"): ItemInstance {
  ...
  if (itemKind === "amulet") {
    ...
    for (let index = 1; index < rollCount; index += 1) { const roll = pool.splice(Math.floor(source.next() * pool.length), 1)[0]; const scale = (level + 1) / 51;
      ...
      else if (roll === "healthOnKill") accessoryBonuses.healthOnKill = 1 + Math.floor(source.next() * 25);
      else if (roll === "manaOnKill") accessoryBonuses.manaOnKill = 1 + Math.floor(source.next() * 50);
...
export function levelUpItem(base: ItemInstance, seed: number): ItemInstance {
  const promotedRarity = base.level >= MAX_ITEM_LEVEL[base.rarity] ? nextRarity(base.rarity) : undefined; const rarity = promotedRarity ?? base.rarity; const nextLevel = promotedRarity ? 1 : Math.min(base.level + 1, MAX_ITEM_LEVEL[rarity]);
  if (base.itemKind === "buckler") {
    const next = generateBuckler(nextLevel, rarity, seed);
    return preserveRolledTraits(base, { ...next, name: base.name, blockChance: 0.1 * RARITY_POWER[rarity], sellValue: Math.max(1, Math.round((nextLevel + 1) * RARITY_POWER[rarity] * (base.reflectionComponents.length ? 5 : 4))) });
  }
  if (base.definitionId === "scepter") return preserveRolledTraits(base, buildWeapon("scepter", nextLevel, rarity, seed, [], seed % 1e8));
  if (base.itemKind === "relic") { const next = generateRelic(nextLevel, rarity, seed); const scaleRatio = weaponLevelScale(nextLevel) / weaponLevelScale(base.level); next.modifiers.lifeStealBase = (base.modifiers.lifeStealBase ?? 0) * scaleRatio; next.modifiers.strengthRegenMultiplier = (base.modifiers.strengthRegenMultiplier ?? 0) * scaleRatio; return preserveRolledTraits(base, { ...next, name: base.name }); }
  if (base.itemKind === "amulet" || base.itemKind === "charm") return preserveRolledTraits(base, { ...generateAccessory(nextLevel, rarity, seed, base.itemKind), name: base.name });
  const next = buildWeapon(base.definitionId as WeaponClass, nextLevel, rarity, seed, [...base.affixes], seed % 1e8); const scaleRatio = weaponLevelScale(next.level) / weaponLevelScale(base.level); next.modifiers.lifeStealBase = (base.modifiers.lifeStealBase ?? 0) * scaleRatio; next.modifiers.strengthRegenMultiplier = (base.modifiers.strengthRegenMultiplier ?? 0) * scaleRatio; return preserveRolledTraits(base, next);
}
...
export function changeItemRarity(base: ItemInstance, rarity: Rarity, seed: number): ItemInstance {
  ...
  const next = buildWeapon(base.definitionId as WeaponClass, level, rarity, seed, [...base.affixes], seed % 1e8); next.modifiers.lifeStealBase = base.modifiers.lifeStealBase; next.modifiers.strengthRegenMultiplier = base.modifiers.strengthRegenMultiplier; return next;
}
...
export function itemKillRestoration(stats: Stats, ...items: Array<ItemInstance | undefined>): { health: number; mana: number } {
  return items.reduce((total, item) => {
    const effectiveness = item ? itemRequirementMultiplier(item, stats) : 1;
    total.health += (item?.accessoryBonuses?.healthOnKill ?? 0) * effectiveness;
...
function preserveRolledTraits(base: ItemInstance, next: ItemInstance): ItemInstance {
  const levelRatio = (next.level + 1) / Math.max(1, base.level + 1);
  const requirements = Object.fromEntries(Object.entries(base.requirements).map(([key, value]) => [key, Math.max(value ?? 0, Math.ceil((value ?? 0) * levelRatio))])) as Partial<Record<StatKey, number>>;
  return { ...next, name: base.name, affixes: [...base.affixes], requirements, statBonuses: upgradedStatBonuses(base, next.seed), skills: [...base.skills], reflectionComponents: [...base.reflectionComponents], attractionSpeed: base.attractionSpeed, perks: { ...base.perks }, immunities: [...(base.immunities ?? [])], accessoryBonuses: base.accessoryBonuses ? structuredClone(base.accessoryBonuses) : undefined };
}
function rollItemPerks(item: ItemInstance, seed: number): ItemInstance { const source = new SeededRandom(seed + 7919); const count = ({ common: 1, uncommon: 2, rare: 3, epic: 4 } as const)[item.rarity]; const pool = [...ITEM_PERKS]; const perks: Partial<Record<ItemPerkId, number>> = {}; const immunities: ItemImmunity[] = []; const requirements = { ...item.requirements }; const max: Record<ItemPerkId, number> = { defense: 10, physicalResist: .5, magicResist: .5, fireResist: .5, frostResist: .5, poisonResist: .5, bleedResist: .5, dodgeChance: .5 }; const immunityFor: Partial<Record<ItemPerkId, ItemImmunity>> = { physicalResist: "physical", magicResist: "magic", fireResist: "fire", frostResist: "frost", poisonResist: "poison", bleedResist: "bleed" }; const attrs: Record<ItemPerkId, StatKey[]> = { defense: ["strength"], physicalResist: ["strength", "agility"], magicResist: ["intelligence"], fireResist: ["intelligence", "magic"], frostResist: ["intelligence"], poisonResist: ["spirit", "agility"], bleedResist: ["strength", "agility"], dodgeChance: ["agility"] }; for (let i=0;i<count;i+=1) { const key=pool.splice(Math.floor(source.next()*pool.length),1)[0]; const factor=((item.level + 1)/51) * (.5 + .5*source.next()); const immunity = immunityFor[key]; if (item.level >= 25 && immunity && source.next() < .1) immunities.push(immunity); else perks[key]=max[key]*factor; const need=Math.ceil(5*item.level*factor/attrs[key].length); for(const attr of attrs[key]) requirements[attr]=Math.max(requirements[attr] ?? 0, need); } return ensureUpgradableAttribute({ ...item, perks, immunities, requirements }, seed); }
function ensureUpgradableAttribute(item: ItemInstance, seed: number): ItemInstance {
...
function upgradedStatBonuses(item: ItemInstance, seed: number): Partial<Record<StatKey, number>> {
  if (!STAT_KEYS.some((key) => (item.statBonuses[key] ?? 0) > 0)) return ensureUpgradableAttribute(item, seed).statBonuses;

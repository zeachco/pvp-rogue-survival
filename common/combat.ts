...
export function skillRange(skill: SkillId, item: ItemInstance, level = 1, spirit = 0): number { const base = SKILLS[skill].range ?? weaponRange(item); return base + Math.min(300, 0.5 * Math.max(1, level) * Math.max(0, spirit)); }

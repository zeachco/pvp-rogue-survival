...
export function sunburnInterval(spirit: number): number {
	return Math.max(0.5, 5 - Math.max(0, spirit) * 0.045);
}
export function sunburnFraction(intelligence: number): number {
	return Math.min(0.1, 0.01 + Math.max(0, intelligence) * 0.0009);
}
export function thunderInterval(level: number): number {
	return 10 - (cappedAuraLevel(level) - 1) * (9 / 98);
}
export function thunderDamage(intelligence: number): number {
	return 3 + Math.max(0, intelligence) * 0.6;
}

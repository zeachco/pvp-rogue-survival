const cappedAuraLevel = (level: number) => Math.max(1, Math.min(100, level));
export function auraRadius(level: number, spirit = 0): number { const capped = cappedAuraLevel(level); return 180 + (capped - 1) * (120 / 99) + Math.min(300, 0.5 * capped * Math.max(0, spirit)); }
export function auraSlowMultiplier(level: number): number { return 0.8 - (cappedAuraLevel(level) - 1) * (0.3 / 99); }
...
export function thunderInterval(level: number): number { return 10 - (cappedAuraLevel(level) - 1) * (9 / 99); }

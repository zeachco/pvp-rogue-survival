...
- A server-authored wave marks entry into a newly requested solo realm or a newly matched competitive realm. Before that wave is queued, the client clears the previous arena, restores the hero to full health, mana, and stamina, removes every bleed, poison, burn, stun, shock, freeze, and curse instance, clears reactive/block and temporary-surge state, stops residual velocity, and resets hero attack and skill cooldowns. Ordinary later wave dispatches inside the same realm do not restore resources or statuses.
...
- Standard projectiles have radius 11, speed 245 px/s, lifetime 4 seconds, and disappear beyond a 40px arena margin. Vampiric Boomerang is three times as broad at radius 33, travels both legs at 180 px/s, and may resolve every eligible overlap in one update. While overlapping an enemy, it deals its cast damage per second continuously, scaled by the exact overlap time in each fixed update, and applies half the normal linear projectile impact force. Orbiting Hammers live `2.4 + 27.6 * (skillLevel - 1) / 98` seconds (2.4 seconds at level 1, 30 seconds at level 99), rotate at 5.2 radians/second, and expand linearly from radius 28 to 190 during their first 2.4 seconds. Frozen Orb moves at 75 px/s for 4 seconds, emits eight radial single-hit spikes every 0.45 seconds, and does not disappear when its orb overlaps an enemy; its spikes travel at 235 px/s for 1.2 seconds. Other projectiles are single-hit.
- A basic or non-Flurry skilled attack recovers in `1 / attacksPerSecond`; Flurry uses `0.35 / attacksPerSecond`. Physical skills add 0.35 stamina to the weapon's normal cost. Most magical skills cost 1 mana; Orbiting Hammers costs 3, Rapid Regeneration costs 4, Force Field costs 8, and Frozen Orb costs 10. Force Field always pushes outward regardless of the main hand. Reflective Surge costs 3 stamina. Healing costs 2 mana, triggers only below 75% HP, and restores its level-scaled fraction of current HP plus `5% * maxHP` and `5% * maxHP * currentStamina / maxStamina`. Rapid Regeneration is a self-buff that only casts while its caster is missing health and has no active instance; it multiplies all normal/equipment health regeneration and adds its flat HP/second value for its level-scaled duration.
- Standard melee attacks and their physical skills use radial force. Standard projectile attacks, Rending Throw, Arcane Bolt, and Frozen Orb spikes use linear force. Force Field is a special radial velocity replacement. Fire Breath, Gooey Swamp, Sunburn, poison, burn, bleed, electric auras, reflection, and every other periodic or passive damage source have no force. Status procs roll independently on a direct weapon hit. Throwing axes have a base 15% bleed chance before compatible affixes are added, and Rending Throw guarantees one standard bleed in addition to independently rolled procs. Bleed lasts 3 seconds at 0.25 damage/second; poison lasts 8 seconds at `0.2 + 0.02 * targetSpirit` damage/second; stun lasts 0.7 seconds; and Freeze lasts 4 seconds. Entering Freeze cancels self-propelled velocity, frozen creeps cannot steer, and any velocity subsequently applied by an impact or Force Field slides them without friction until Freeze ends; a hit that both freezes and applies force preserves that force. Applying a Freeze removes one active Burn stack, and applying a Burn removes one active Freeze stack, one-for-one before its own stack is added. Bash always stuns 1.1 seconds, Shockwave 0.6, Sweep bleeds for 3 seconds at 0.35/second, Cleave bleeds for 2 seconds at 0.45/second, and Arcane Bolt applies Freeze. Frozen Orb and each emitted spike apply Freeze. Status instances stack as separate records, periodic instances tick once per second, and any active stun prevents steering.

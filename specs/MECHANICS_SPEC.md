...
- Units decelerate when they have no desired movement. Force Field never teleports a creep: it deals `0.2 * spellPower(level)` magic damage and replaces the creep's current velocity with a 180 px/s radial launch outward from the hero, regardless of the equipped main hand. Existing inward momentum therefore cannot weaken or reverse the push. Applying the launch interrupts the target creep's current attack: internal ranged wind-up is cleared and an unresolved melee/area telegraph from that creep is invalidated. Already-launched projectiles remain independent and continue.
- Force Field affects creeps only. It never moves uncollected equipment, Gold, or Scrap.

...
- Rendering draws the fixed arena, hero, creeps, attack areas, projectiles, health bars, combat hints, filled-diamond equipment drops, yellow circular Gold drops, and hollow-diamond Scrap drops. Active statuses visibly affect their unit: Freeze has highest tint priority and applies a light-blue tint; then Burn applies a fire tint; Poison applies a light-green tint; and Curse applies a dark-purple tint. Bleed does not tint, instead emitting tiny blood particles. Stun and Shock draw a small rotating white asterisk above the unit. The hero HUD renders one compact status icon per active status kind above its health bar; duplicate status instances are aggregated into stacks. Hovering or keyboard-focusing an icon opens a tooltip with its name, longest remaining duration, stack count when applicable, and combined damage per second for damaging statuses. A creep whose health was actually reduced flashes white for exactly its next rendered frame; this is shared hit feedback and applies to direct, status, aura, reflection, and Thorns damage without creating a projectile or spell-effect animation.

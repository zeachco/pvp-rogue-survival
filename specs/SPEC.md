...
- `chatMessage`: `{ senderId, senderName, text }`; sent to all members of a realm when one member sends a chat. Also used by the server at wave dispatch to notify a player that another player's sent creeps arrived in their realm. The client displays `senderName: text` with the name in green when the sender is on the same side (both guard or both attacker) and in red when they are an opponent.
...
- The world is 1,600 by 1,000 logical pixels. The camera viewport equals the canvas CSS content size and always centers on the hero. When the viewport is smaller than the arena the hero is kept at center as long as the arena edges remain visible; when the viewport is larger than the arena the map is centered on screen. Canvas backing width and height equal CSS dimensions multiplied by device pixel ratio, and the 2D context transform restores logical-pixel drawing. Both window resize and `ResizeObserver` trigger this calculation.

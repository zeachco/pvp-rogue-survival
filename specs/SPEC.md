...
- Cooldown spells use two compact Ubuntu-dock-like vertical rails on the left edge, anchored above the bottom-left health display. The Learned Skills rail contains every permanently learned skill; the Geared Skills rail immediately to its right contains skills granted by equipped gear that are not yet learned. Their circular icons grow upward. These two rails are the complete activation surface: a skill outside both rails never triggers.
...
- `reorderSkills`: `{ skillOrder }`; persists the complete bottom-first skill priority after a drag swap.

...
- A newly created hero begins with the Character, Inventory, and Available Spells panels open so the starter build and choices are immediately visible. Clicking Enter Realm collapses the Character and Inventory panels if open and closes the Available Spells panel before matchmaking begins. Reconnecting an existing hero retains the ordinary persisted onboarding-trigger behavior rather than forcibly reopening all three panels.

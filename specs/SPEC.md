...
- The spell catalog has two persistent three-state filters that apply simultaneously as an intersection. Type selects Both, Active, or Passive. Learning selects Both, Learned, or Not learned; for example, Passive plus Learned shows only learned passives. A spell counts as Learned only when its permanent extracted level is positive and any authored hero-level requirement is currently met. Empty resource sections hide while either or both filters are active.

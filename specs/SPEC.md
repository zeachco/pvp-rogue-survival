...
- Use a 320px independently scrolling character/stat panel and a 640px equipment panel as floating canvas overlays. Each panel has its own small arrow toggle and retracts independently to a 30px tab; `C` toggles the character panel, while either `I` or `V` toggles inventory when keyboard focus is not in an editable control. The inventory spans from the viewport top to a 16px bottom inset with a 200px minimum height. Its currencies and equipment count remain fixed at the top while only the equipment list scrolls. Committing a creep selection forces the character panel open and replaces its contents with that creep; clearing the selection restores the local hero and the panel's exact pre-selection open/closed state. Temporary pointer inspection does not change that saved state.
